...
const NamadaPeers = dynamic(() => import("@/src/widgets/services-content/namada/namada-peers/NamadaPeers"))

const TendermintPeers:FC<TendermintContentProps> = ({network, chainId}) => {
    const networkParams = useTendermintNetworkParams(network.name, network.type);

    if (network.name === "namada") {
        return (
            <NamadaPeers network={network} chainId={chainId} />
        );
    }
